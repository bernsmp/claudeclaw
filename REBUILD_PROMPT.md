...
Every Telegram chat maps to a Claude Code session ID stored in SQLite. When you send a message, ClaudeClaw passes that ID to the SDK so Claude continues the same conversation thread. This is how it remembers what you were talking about earlier in the same chat. `/newchat` clears the active Claude session, but durable memory can still be recalled later. It is a fresh thread, not a blank-memory reset.
